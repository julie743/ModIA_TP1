...
class DownSampleBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.maxpool(torch.tensor(x_skip))
        return out , x_skip
...
class UpSampleBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.conv_block = double_conv(in_channels, out_channels)
        self.upsample = nn.Upsample(scale_factor=2,mode='nearest') # Upsample multilie la taille de l'image par un facteur
...
class UNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.upsample_block_3 = UpSampleBlock(256+128,128) #on fait +128 car il faut prévoir la flèche en pointillé venant du x_skip
